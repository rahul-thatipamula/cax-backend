...
VM_IP="${DEPLOY_VM_IP:-144.24.142.198}"
VM_USER="${DEPLOY_VM_USER:-opc}"
...
# Fix and validate private key path/permissions
if [ ! -f "$KEY_PATH" ]; then
...
ssh_cmd() {
    ...
    ssh -i "$KEY_PATH" -o StrictHostKeyChecking=accept-new "$VM_USER@$VM_IP" "$@"
}
