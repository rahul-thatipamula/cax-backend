...
import com.cax.cax_backend.organization.service.OrganizationPostService;
...
public class OrganizationPostController {
    ...
    private final OrganizationPostService clubPostService;
    ...
    @PostMapping(value = "/{organizationId}/posts/upload-images", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<ApiResponse<List<String>>> uploadPostImages(
        ...
        if (!clubPostService.canManagePosts(userId, organizationId)) {
        ...
        String folder = "club-posts/" + organizationId;
        for (MultipartFile file : files) {
    ...
    public ResponseEntity<ApiResponse<List<OrganizationPost>>> getFeed(
            Authentication auth,
    ...
    public ResponseEntity<ApiResponse<List<OrganizationPost>>> getOrganizationPosts(@PathVariable String organizationId) {
    ...
    public ResponseEntity<ApiResponse<OrganizationPost>> createPost(
        ...
        String userId = (String) auth.getPrincipal();
        OrganizationPost post = clubPostService.createPost(
                userId,
                organizationId,
                request.getCaption(),
                request.getImages(),
    ...
    public ResponseEntity<ApiResponse<OrganizationPost>> votePoll(
            Authentication auth,
            @PathVariable String postId,
            @RequestParam String optionId) {
        String userId = (String) auth.getPrincipal();
        OrganizationPost post = clubPostService.votePoll(userId, postId, optionId);
    ...
    public ResponseEntity<ApiResponse<OrganizationPost>> likePost(Authentication auth, @PathVariable String postId) {
        String userId = (String) auth.getPrincipal();
        OrganizationPost post = clubPostService.likePost(userId, postId);
        return ResponseEntity.ok(ApiResponse.success("Like updated successfully", post));
    }

    @PostMapping("/posts/{postId}/comments")
    public ResponseEntity<ApiResponse<OrganizationPost>> addComment(
            Authentication auth,
            @PathVariable String postId,
            @RequestBody AddCommentRequest request) {
        String userId = (String) auth.getPrincipal();
        OrganizationPost post = clubPostService.addComment(userId, postId, request.getText());
        return ResponseEntity.ok(ApiResponse.success("Comment added successfully", post));
    }

    @DeleteMapping("/posts/{postId}/comments/{commentId}")
    public ResponseEntity<ApiResponse<OrganizationPost>> deleteComment(
            Authentication auth,
            @PathVariable String postId,
            @PathVariable String commentId) {
        String userId = (String) auth.getPrincipal();
        OrganizationPost post = clubPostService.deleteComment(userId, postId, commentId);
